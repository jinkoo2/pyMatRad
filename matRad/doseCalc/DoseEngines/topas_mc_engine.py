...
def _write_ct_binary(path: str, hu_cube: np.ndarray):
    """Write CT HU cube as int16 binary in Fortran order (TOPAS ImageCube)."""
    arr = np.clip(hu_cube, -1024, 3071).astype(np.int16)
...
class TopasMCEngine(DoseEngineBase):
    ...
    def _write_patient(self, ct: dict, hu_cube: np.ndarray):
        """Write CT geometry parameter file + binary HU data."""
        ...
        txt_path = os.path.join(self.working_dir, "matRad_cube.txt")

        _write_ct_binary(dat_path, hu_cube)

        lines = [
            "# matRad CT geometry — auto-generated by pyMatRad TopasMCEngine",
            f"s:Ge/Patient/Type               = \"TsImageCube\"",
            f"s:Ge/Patient/Parent             = \"World\"",
            f"s:Ge/Patient/InputDirectory     = \"{self.working_dir}\"",
            f"s:Ge/Patient/InputFile          = \"matRad_cube.dat\"",
            f"s:Ge/Patient/ImagingToMaterial  = \"Schneider\"",
            ...
            "",
            "# Schneider HU-to-material converter (3-tissue approximation)",
            "s:Ma/Schneider/Type              = \"HUtoMaterial\"",
            "dv:Ma/Schneider/HounsfieldLower  = 4 -1000 -950 -700 101 Hu",
            "dv:Ma/Schneider/HounsfieldUpper  = 4 -950  -700  101 3000 Hu",
            "sv:Ma/Schneider/Material         = 4 "
    ...
    def _write_beam_file(self, beam: dict, beam_idx: int, out_path: str,
        ...
        """Write a TOPAS parameter file for one beam (all bixels, uniform weight)."""
        sad  = float(beam["SAD"])
        ...
        ny, nx, nz = (ct.get("cubeDim") or
                      [len(ct["y"]), len(ct["x"]), len(ct["z"])])
        rx = ct["resolution"]["x"]
        ry = ct["resolution"]["y"]
        rz = ct["resolution"]["z"]

        # Convert gantry / couch to TOPAS rotation convention
        # TOPAS: source at (0, sad, 0) in beam-nozzle frame;
        # gantry rotation around Z, couch around Y.
        gantry_rad = np.radians(ga)
        couch_rad  = np.radians(ca)

        # Source position in world (TOPAS uses cm; we write mm here with "mm")
        src_world = (np.asarray(beam["sourcePoint"]) - iso)

        lines = [
            f"# TOPAS beam {beam_idx+1}: gantry={ga}°  couch={ca}°",
            f"includeFile = {self.working_dir}/matRad_cube.txt",
            "",
            "# ---- World / geometry ----------------------------------------",
            ...
            "",
            "# ---- Scorer --------------------------------------------------",
            f"s:Sc/Dose/Quantity       = \"DoseToMedium\"",
            f"s:Sc/Dose/Component      = \"Patient\"",
            f"b:Sc/Dose/OutputToConsole = \"False\"",
            f"s:Sc/Dose/OutputType     = \"Binary\"",
            f"s:Sc/Dose/IfOutputFileAlreadyExists = \"Overwrite\"",
            f"s:Sc/Dose/OutputFile     = \"{result_prefix}\"",
            "",
            "# ---- Physics -------------------------------------------------",
            "ph:Ph/Default/Modules = 1 \"g4em-standard_opt4\"",
            "",
            "# ---- Run control --------------------------------------------",
            f"i:Ts/Seed = 97",
