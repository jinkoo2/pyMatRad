...
def save_dij(plan_name: str, dij: dict, stf: list, cache_root: str):
    ...
    d = _cache_dir(plan_name, cache_root)
    # Save sparse dose matrix separately (scipy npz)
    sp.save_npz(os.path.join(d, "dij_matrix.npz"), dij["physicalDose"][0].tocsc())
    # Save dij metadata and stf via pickle (exclude the large sparse matrix)
    dij_meta = {k: v for k, v in dij.items() if k != "physicalDose"}
    with open(os.path.join(d, "dij_meta.pkl"), "wb") as f:
        pickle.dump({"dij_meta": dij_meta, "stf": stf}, f,
                    protocol=pickle.HIGHEST_PROTOCOL)
    print(f"  Saved dij → {d}/dij_matrix.npz + dij_meta.pkl")


def load_dij(plan_name: str, cache_root: str):
    """Load dij and stf from cache.  Returns (dij, stf) or (None, None)."""
    d = os.path.join(cache_root, plan_name)
    mat_path  = os.path.join(d, "dij_matrix.npz")
    meta_path = os.path.join(d, "dij_meta.pkl")
    ...
    stf = data["stf"]
    print(f"  Loaded dij ← {mat_path}  shape={matrix.shape}")
    return dij, stf


def save_result(plan_name: str, mode: str, dose: np.ndarray,
                w: np.ndarray, cache_root: str):
    """Save dose result (physicalDose cube + weight vector) to cache."""
    d = _cache_dir(plan_name, cache_root)
    path = os.path.join(d, f"result_{mode}.npz")
...
def load_result(plan_name: str, mode: str, cache_root: str):
    ...
    path = os.path.join(os.path.join(cache_root, plan_name), f"result_{mode}.npz")
...
def run(plan_name: str, ct_dir: str = None, calc_dose: bool = True,
    ...
    if cached is not None:
        ct, cst, pln, dose_eclipse = (cached["ct"], cached["cst"],
                                      cached["pln"], cached["dose"])
    else:
        result = import_dicom(plan_dir, ct_dir=ct_dir_abs)
        ct, cst, pln, dose_eclipse = (result["ct"], result["cst"],
                                      result["pln"], result["dose"])
        save_import(plan_name, ct, cst, pln, dose_eclipse,
                    result.get("dose_grid"), cache_root)
    ...
    # ── 3. Generate STF + dij (load from cache if available) ─────────────
    dij, stf = (None, None) if force else load_dij(plan_name, cache_root)
    if dij is None:
        ...
        save_dij(plan_name, dij, stf, cache_root)
    else:
        total_bixels = sum(b["totalNumOfBixels"] for b in stf)
        print(f"  {len(stf)} beams, {total_bixels} bixels total (from cache)")

    mode = "eclipse_fluence" if eclipse_fluence else "reoptimised"
    dose_matrad, w_cached = (None, None) if force else load_result(plan_name, mode, cache_root)
    ...
    if dose_matrad is None:
        ...
        save_result(plan_name, mode, dose_matrad, w, cache_root)
    else:
        w = w_cached
